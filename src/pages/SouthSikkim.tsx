...
import { ArrowLeft, MapPin, Star, Eye, MessageCircle, Church, Crown } from 'lucide-react';
...
const SouthSikkim = () => {
  ...
  const regionData = regionsData.find(region => region.id === 'south')!;
  ...
  const openChatBot = (monasteryName: string) => {
    setChatBot({ isOpen: true, context: `${monasteryName} in South Sikkim` });
  };
        ...
        <div ref={headerRef} className="px-6 py-16">
          <div className="container mx-auto">
            ...
            <div className="flex items-start gap-4 mb-6">
              <Church className="w-12 h-12 text-primary mt-2" />
              <div>
            ...
            <div className="flex items-center gap-4">
              <div className="glass rounded-full px-4 py-2 flex items-center gap-2">
                ...
                <span className="text-sm font-medium">Massive Statues</span>
              </div>
                ...
                <Card className={`overflow-hidden border-0 glass hover:shadow-large transition-all duration-500 ${
                  ...
                  <div className="flex-1 relative overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4 glass rounded-full px-3 py-1 flex items-center gap-1">
                      ...
                      <span className="text-white font-medium text-sm">4.7</span>
                    </div>
                    {monastery.id === 'buddha-park' && (
                      <div className="absolute top-4 right-4 bg-gradient-mountain rounded-full px-3 py-1">
                        <span className="text-white text-xs font-medium">130ft Statue</span>
                      </div>
                    )}
                    {monastery.id === 'samdruptse' && (
                      <div className="absolute top-4 right-4 bg-purple-500/20 backdrop-blur-sm rounded-full px-3 py-1">
                        <span className="text-purple-300 text-xs font-medium">World's Largest</span>
                      </div>
                    )}
                    {monastery.id === 'bon' && (
                      <div className="absolute top-4 right-4 bg-orange-500/20 backdrop-blur-sm rounded-full px-3 py-1">
                        <span className="text-orange-300 text-xs font-medium">Unique Tradition</span>
                      </div>
                    )}
                  </div>
